...
function createApp({ environment = {}, storedOptions = {}, installer = createInstaller(), platform = 'linux', survivingTransaction = null, desktopUpdater } = {}) {
  const app = express();
  const processMock = {
    env: environment,
    platform,
...
describe('OpenChamber desktop host update route', () => {
  it('reports a restart rejection until the user retries installation', async () => {
    ...
    const { app, installer } = createApp({ environment: { OPENCHAMBER_RUNTIME: 'desktop' }, desktopUpdater });
  ...
  it('rejects native checks without a bridge and preserves explicit non-web checks', async () => {
    const { app, installer } = createApp({ environment: { OPENCHAMBER_RUNTIME: 'desktop' } });
