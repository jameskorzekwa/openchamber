...
- On narrow screens the dashboard fills `100dvh` and `100vw`, removes rounded dialog chrome, and clips horizontal overflow. Its sticky, opaque, z-raised header owns the title and close button and respects `env(safe-area-inset-top)`. Bottom padding respects `env(safe-area-inset-bottom)`. Existing Sonner notices stay alive below the shared dialog layer, so they reappear after close without covering required dialog controls.
- The dashboard starts with an authoritative five-state overview and a total derived from the visible tree. Supervisor diagnostics are collapsed separately below it.
- Every task is a compact two-line summary with a phase badge and age. Details stay collapsed until tapped on every viewport size. Parent and child labels are explicit, child summaries remain visible while the parent detail is collapsed, and needs-owner rows appear only once in the tree. Titles truncate in summaries; reasons and guidance wrap with `break-words`/`overflow-wrap:anywhere`; only monospace sha/command/tech spans use `break-all`.
